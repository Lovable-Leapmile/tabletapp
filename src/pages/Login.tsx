...
const Login = () => {
  ...
  const loginBgStyle = isDhlTheme 
    ? 'bg-gradient-to-b from-[#FFCC00] via-[#FFD633] to-[#FFCC00]'
    : '';

  const buttonStyle = isDhlTheme
    ? { 
        backgroundColor: '#FFCC00',
        boxShadow: '0 4px 14px rgba(255, 204, 0, 0.4), 0 1px 0 rgba(255, 255, 255, 0.3) inset',
      }
    : {
        backgroundColor: `hsl(${skin.colors.primary})`,
        boxShadow: `0 4px 14px hsl(${skin.colors.primary} / 0.4), 0 1px 0 rgba(255, 255, 255, 0.3) inset`,
      };

  const inputFocusStyle = isDhlTheme
    ? 'focus:border-[#FFCC00] focus:ring-[#FFCC00]/20'
    : 'focus:border-primary focus:ring-primary/20';

  return (
    <div 
      className={`min-h-screen flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden ${loginBgStyle}`}
      style={!isDhlTheme ? { backgroundColor: `hsl(${skin.colors.loginBackground})` } : undefined}
    >
      {/* Technical Warehouse Blueprint Pattern - Only for DHL theme */}
      {isDhlTheme && (
      ...
      {/* Subtle pattern for non-DHL themes */}
      {!isDhlTheme && (
      ...
      <div className="w-full max-w-md space-y-10 sm:space-y-12 animate-fade-in relative z-10">
        ...
        <div className="text-center">
          <img 
            src={skin.logoUrl} 
            alt={skin.name} 
            className="h-14 sm:h-18 mx-auto opacity-95 max-w-[200px] object-contain" 
          />
        </div>
        ...
        >
          ...
          >
            ...
            <div className="flex justify-center mb-8">
              ...
              >
                <Lock 
                  className="w-10 h-10 sm:w-12 sm:h-12 animate-lock-pulse" 
                  strokeWidth={2}
                  style={{ color: isDhlTheme ? '#374151' : `hsl(${skin.colors.primary})` }}
                />
                {/* Subtle glow */}
                <div 
            ...
            <div className="space-y-5">
              ...
              <div className="relative">
                <Input 
                  ...
                  className={`h-14 sm:h-16 text-lg sm:text-xl text-center font-medium bg-gray-50/80 border-gray-200/80 focus:bg-white transition-all duration-200 rounded-2xl focus:ring-2 ${inputFocusStyle}`}
                  style={{
              ...
              <Button 
                onClick={handleLogin} 
                className={`w-full h-14 sm:h-16 text-lg sm:text-xl font-semibold rounded-2xl transition-all duration-200 shadow-lg hover:shadow-xl ${isDhlTheme ? 'text-gray-900 hover:bg-[#FFD633]' : 'text-primary-foreground hover:opacity-90'}`}
                style={buttonStyle}
                disabled={isLoading}
              >
        ...
        {/* Footer */}
        <p className={`text-center text-xs sm:text-sm font-normal ${isDhlTheme ? 'text-black' : 'text-white/90'}`}>
          Powered by <span className="font-medium">{skin.name}</span> © 2025
        </p>
      </div>
    </div>
  );
